...
it('calls injected function on array member', () => {
  ...
  const set = new MutableMultiKey<[number, number]>(Set, equal)
...
it('add() works as intended', () => {
  const element = [0, 1, 2]
  const set = new MutableMultiKey(Set)
...
it('has() works as intended', () => {
  const set = new MutableMultiKey<number[]>(Set)
  set.add([0, 0])
  set.add([0, 1])
...
it('elements are mutable', () => {
  ...
  expect({
    ...
    beforeMutation: [true, false],
    afterMutation: [false, true]
  })
})
